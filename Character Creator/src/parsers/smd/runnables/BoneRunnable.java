    ...
    public void run() {
        ...
        if (scanner != null) {
            ...
            while (scanner.hasNextLine()) {
                ...
                if (s.split("\\s")[0].equals(SMDConstants.SMD_VERSION_HEADER)) {
                    model.setVersion(Integer.parseInt(s.split("\\s")[1]));
                } else if (s.split("\\s")[0].equals(SMDConstants.SMD_NODES_HEADER)) {
...
//                        System.out.println(bone.getBoneID() + " " + bone.getBoneName() + " " + bone.getParentBoneID());
                    ...
                    s = scanner.nextLine();
                    if (s.split("\\s")[0].equals(SMDConstants.SMD_BONE_POSITIONS_HEADER)) {
                        model.setTimeframe(Integer.parseInt(s.split("\\s")[1]));
                        s = scanner.nextLine();
